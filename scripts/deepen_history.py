...
Usage:
  ...
  python scripts/deepen_history.py                  # probe + ingest if deep enough
...
def probe_depth(api_key: str, start: str, end: str, ticker: str = PROBE_TICKER) -> tuple[int, date | None, date | None, str]:
    """Return (n_bars, first, last, status_note) for one aggregates request."""
    url = (
...
def main() -> int:
    ...
    if wf_ready:
        ...
        print(
            ...
            f"need Massive Stocks Starter ($29/mo, 5y history) or higher. "
            f"Current plan returns {n} bars (need >= {MIN_SESSIONS_FOR_DEFAULT_WF})."
        )
        print("  Pricing: https://massive.com/pricing")
